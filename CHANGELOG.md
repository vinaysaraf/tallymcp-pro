...
## v1.0.8 — Report-correctness fixes: voucher amounts, Balance Sheet, `[object Object]` crash (2026-06-23)
...
- **Day Book & Sales Register were missing amount, party, ledger, reference and narration.** A raw `Voucher` collection does not populate method-backed fields (`$Amount`, `$PartyLedgerName`, `$Narration`, `$Reference`) unless they are explicitly fetched — so they came back empty/0. Fixed by adding them to `<FETCH>`. The amount field additionally had a malformed expression (`$$StringFindAndReplace:$$Number:$$String:$Amount:...` — missing parentheses around the numeric sub-expression, so TDL mis-parsed the `:`-delimited arguments and returned empty); corrected to `$$StringFindAndReplace:($$Number:$Amount):"(-)":"-"` (the working Trial Balance pattern). The **Sales Register** now shows Party, Reference, Amount and Narration, and the **Sales Trend** dashboard (which sums sales-voucher amounts) shows real figures instead of zero.
- **Day Book now shows full per-line ledger detail.** A TDL `Voucher` projection can only expose one ledger per voucher, so the Day Book is now read via the memory-safe raw-voucher stream (the same path audit-lite uses), which carries every `<ALLLEDGERENTRIES.LIST>` posting. The Excel export explodes each voucher into **one row per ledger line** — `Date, Type, Number, Party, Ledger, Debit, Credit, Narration` — splitting the entry amount into Debit/Credit by `isDeemedPositive` (true = debit). The JSON export keeps the nested voucher structure with the full `entries[]` breakdown. The now-unused inline-TDL `day-book.xml` template and its catalog entry were removed.
- **Balance Sheet double-counted and never tied out.** The connector summed *every* non-revenue group — primary groups **and** their sub-groups — but a parent's closing already aggregates its children, so the totals were inflated and the sheet never balanced. It now keeps only primary (top-level) groups and appends a single **Profit & Loss A/c** balancing line equal to `-(sum of the other primary groups)` — which, by double-entry, is exactly Tally's combined brought-forward + current-period P&L figure. The signed Amount column now ties to zero. (Verified live: Assets −9,08,252.87 = Liabilities 9,08,252.87.) Empty `Sub Group`/`Ledger` columns were dropped from the layout.
