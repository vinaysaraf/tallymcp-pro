...
- **Day Book & Sales Register amounts were always `0`.** Two TDL bugs combined: (1) the amount field read `$Amount` on a raw `Voucher` collection, where it is unpopulated unless explicitly fetched — fixed by adding `<FETCH>Amount</FETCH>` to the collection; (2) the field expression `$$StringFindAndReplace:$$Number:$$String:$Amount:...` was missing parentheses around the numeric sub-expression, so TDL mis-parsed the `:`-delimited arguments and returned empty — fixed to `$$StringFindAndReplace:($$Number:$Amount):"(-)":"-"` (matching the working Trial Balance pattern). The net voucher value now surfaces as a new **Amount** column in both exports, and the **Sales Trend** dashboard (which sums sales-voucher amounts) now shows real figures instead of zero.
